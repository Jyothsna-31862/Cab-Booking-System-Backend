...
class RatingServiceTests {
    ...
    void createRating_shouldReturnRatingDTO_whenValidDataProvided() {
        ...
        verify(driverServiceClient, times(1)).updateDriverRating("200", 4.5);
    }
    ...
    void createRating_shouldAcceptAllValidScores_fromOneToFive() {
        ...
        when(ratingRepository.findAverageRatingByDriverId(anyInt())).thenReturn(4.0);
        when(driverServiceClient.updateDriverRating(anyString(), anyDouble()))
                .thenReturn(ResponseEntity.ok().build());
    ...
    void createRating_shouldHandleDriverServiceFailure_gracefully() {
        ...
        verify(ratingRepository, times(1)).save(any(Rating.class));
        verify(driverServiceClient, times(1)).updateDriverRating(anyString(), anyDouble());
    }
